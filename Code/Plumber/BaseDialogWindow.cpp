...
namespace Plumber
{
	...
	{
		return reinterpret_cast<BaseDialogWindow *>(GetWindowLong(hWnd, GWLP_USERDATA));
	}
		...
		LPARAM lParam)
	{
		...
		case WM_INITDIALOG:
		{
			SetWindowLong(hwnd, GWLP_USERDATA, lParam); // setting window user data as window pointer
	...
	POINT BaseDialogWindow::GetLocalPosition(HWND hwnd)
	{
		...
		HWND parentHwnd = reinterpret_cast<HWND>(GetWindowLong(hwnd, GWLP_HWNDPARENT));
